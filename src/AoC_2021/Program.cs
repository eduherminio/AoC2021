...
else
{
    ...
    Solver.Solve(indexes.Where(i => i < uint.MaxValue), new SolverConfiguration { ShowConstructorElapsedTime = true, ShowTotalElapsedTimePerDay = true, ShowOverallResults = false, ClearConsole = false });
}